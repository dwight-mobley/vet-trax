...
export default function PetMedicalRecordHistoryTable({
    ...
    return (
        <div>
                ...
                {records.map((record) => (
                    ...
                    >
                        <div className="mb-2 flex items-start justify-between gap-3">
                            <p className="text-sm font-semibold text-text-primary">{record.description || "Medical Record"}</p>
                            <p className="text-xs text-text-secondary">{dateFormatter(record.date)}</p>
                        </div>
                        <div className="mb-2 grid grid-cols-2 gap-2 text-xs text-text-secondary">
                            <p>Vet: {record.vet || "-"}</p>
            ...
            <div className="hidden overflow-x-auto rounded-large border border-text-disabled/30 bg-background-paper shadow-sm md:block">
